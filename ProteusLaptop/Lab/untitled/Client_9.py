...
import random
import ClientBase
from Judge import identifyHand
...
# Agent
POKER_CLIENT_NAME = 'Ace_of_Spades'
CURRENT_HAND = []
...
'''
def queryCallRaiseAction(_maximumBet, _minimumAmountToRaiseTo, _playersCurrentBet, _playersRemainingChips, _hand):
    print("Player requested to choose a call/raise action.")


    def TryCall():
        return ClientBase.BettingAnswer.ACTION_CALL if _playersCurrentBet + _playersRemainingChips > _maximumBet else ClientBase.BettingAnswer.ACTION_ALLIN
    result = identifyHand(_hand)
    print result


    if typeRank[result[0]]<2:

        if _playersCurrentBet + _playersRemainingChips > _minimumAmountToRaiseTo and _maximumBet < 0.2*_playersRemainingChips:
            ...
            return ClientBase.BettingAnswer.ACTION_FOLD if _playersCurrentBet < 0.1 * _playersRemainingChips else TryCall()
    elif typeRank[result[0]]<3:
        if _playersCurrentBet + _playersRemainingChips > _minimumAmountToRaiseTo and _maximumBet < 0.5*_playersRemainingChips:
            return ClientBase.BettingAnswer.ACTION_RAISE, (random.randint(0,10) + _minimumAmountToRaiseTo) if _playersCurrentBet + _playersRemainingChips + 10 > _minimumAmountToRaiseTo else _minimumAmountToRaiseTo
        else:
            return ClientBase.BettingAnswer.ACTION_FOLD if _playersCurrentBet < 0.3 * _playersRemainingChips else TryCall()
    elif typeRank[result[0]]<4:
...
def queryCardsToThrow(_hand):
    ...
    for c in _hand:
        ...
        if handRank[c[0]] < 11 and dirty:
            throwcards = throwcards+str(c)+" "
    return throwcards
